...
local_thresholds = threshold_local(gbs_sharpened, block_size=15)
global_threshold = threshold_otsu(gbs_sharpened)
gbs_thresholded = gbs_sharpened > local_thresholds

gbs_eroding = remove_small_holes(gbs_thresholded, area_threshold=10)

for _ in range(0):
    #gbs_eroding = opening(gbs_eroding, selem=disk(1.5))
    #gbs_eroding = opening(gbs_eroding, selem=disk(3))
    gbs_eroding = binary_erosion(gbs_eroding, selem=disk(1))
